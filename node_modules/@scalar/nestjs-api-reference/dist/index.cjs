...
var ApiReference = (options) => {
  ...
  return `
    <script
      ...
      data-configuration="${JSON.stringify(configuration).split('"').join("&quot;")}">${configuration.spec?.content ? typeof configuration.spec?.content === "function" ? JSON.stringify(configuration.spec?.content()) : JSON.stringify(configuration.spec?.content) : ""}</script>
